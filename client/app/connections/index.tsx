...
export default function ConnectionsScreen() {
  ...
  const renderCard = (definition: PlatformDefinition) => {
    ...
    return <Pressable key={definition.id} accessibilityRole="button" onPress={() => void act(definition)} style={({ pressed }) => ({ width: '48%', minWidth: 154, flexGrow: 1, padding: space[4], gap: space[3], borderRadius: radius.card, borderWidth: 1, borderColor: connected ? colors.success : colors.neutral[200], backgroundColor: colors.paper, opacity: pressed ? 0.72 : 1 })}>
    ...
    <View style={{ flex: 1, backgroundColor: colors.cream }}>
      <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ paddingBottom: 48 }}>
        ...
        {loading ? <ActivityIndicator style={{ marginTop: 80 }} color={colors.ink} /> : error ? <MobileState error title="Connections unavailable" message={error} /> : <View style={{ paddingHorizontal: space[4], gap: space[4] }}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: space[3] }}>{definitions.filter(item => item.supportStatus === 'available' || item.supportStatus === 'beta').map(renderCard)}</View>
          <SectionLabel title="On the roadmap" detail="Vote with a tap" />
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: space[3] }}>{definitions.filter(item => item.supportStatus === 'planned' || item.supportStatus === 'unavailable').map(renderCard)}</View>
        </View>}
      </ScrollView>
