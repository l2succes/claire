...
import { Bell, Bot, ChevronLeft, ChevronRight, DatabaseZap, Link2, LogOut, ShieldCheck, Sparkles } from 'lucide-react-native';
  ...
  return (
    <ScrollView testID="settings-screen" style={{ flex: 1, backgroundColor: colors.cream }} contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ paddingBottom: 64 }}>
      ...
      <View style={{ paddingHorizontal: space[4], gap: space[5] }}>
        ...
        <View style={{ gap: space[2] }}><SectionLabel title="Preferences" />{rows.map(row => <Pressable key={row.title} testID={`settings-${row.title.toLowerCase().replace(/[^a-z]+/g, '-')}`} onPress={() => router.push(row.href as never)} style={({ pressed }) => ({ minHeight: 68, flexDirection: 'row', alignItems: 'center', gap: space[3], padding: space[3], borderRadius: radius.control, backgroundColor: pressed ? colors.sky : colors.paper, borderWidth: 1, borderColor: colors.neutral[200] })}><View style={{ width: 40, height: 40, borderRadius: 13, backgroundColor: row.title === 'AI Settings' ? colors.lavender : row.title === 'Privacy & data' ? colors.sky : colors.neutral[100], alignItems: 'center', justifyContent: 'center' }}><row.icon size={19} color={colors.ink} /></View><View style={{ flex: 1 }}><Text style={{ ...mobileType.body, fontWeight: '700', color: colors.ink }}>{row.title}</Text><Text style={{ ...mobileType.bodySmall, color: colors.neutral[600] }}>{row.detail}</Text></View><ChevronRight size={18} color={colors.neutral[400]} /></Pressable>)}</View>
