...
function DesktopWorkspace({ auth, compactWindow, initialConversationId, session }: { auth: DesktopAuth; compactWindow: boolean; initialConversationId?: string; session: Session }) {
  ...
  const refreshChats = useCallback(async (showLoading = true) => {
    ...
    const request = (async () => {
      ...
      try {
        ...
        desktopCacheRef.current.update({ chats: nextChats, loops: bootstrap.loops, preferences: bootstrap.preferences, cursor: bootstrap.cursor });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!active) return;
      if (savedDestination && ['Home', 'Inbox', 'Loops', 'People', 'Search', 'Settings'].includes(savedDestination)) {
  ...
  return (
    <SafeAreaView style={styles.safeArea} testID="claire-desktop-shell">
      ...
      <View style={styles.appFrame}>
        ...
        {visitedDestinations.has('Home') ? <WorkspacePane active={destination === 'Home'}><HomePane api={api} companion={companion} conversations={conversations} onOpenChat={openConversation} onOpenInbox={() => setDestination('Inbox')} /></WorkspacePane> : null}
        {visitedDestinations.has('Loops') ? <WorkspacePane active={destination === 'Loops'}><LoopsPane api={api} onOpenChat={openConversation} onOpenInbox={() => setDestination('Inbox')} /></WorkspacePane> : null}
        {visitedDestinations.has('People') ? <WorkspacePane active={destination === 'People'}><PeoplePane api={api} conversations={conversations} selectedConversationId={peopleSelectionId} onOpenChat={openConversation} /></WorkspacePane> : null}
        {visitedDestinations.has('Search') ? <WorkspacePane active={destination === 'Search'}><AssistantPane api={api} selected={selected} initialQuestion={globalSearchSeed} onInitialQuestionConsumed={clearGlobalSearchSeed} onOpenMessage={(chatId, messageId) => { setSelectedConversationId(chatId); setHighlightedMessageId(messageId); setDestination('Inbox'); }} /></WorkspacePane> : null}
        {visitedDestinations.has('Settings') ? <WorkspacePane active={destination === 'Settings'}><SettingsPane api={api} companion={companion} companionNotice={companionNotice} apiUrl={auth.config.apiUrl} accessToken={session.access_token} userId={session.user.id} cache={desktopCacheRef.current} fullHistoryEnabled={fullHistoryEnabled} onFullHistoryChange={setFullHistoryEnabled} onNotificationPreferenceChange={setNotificationsEnabled} onSetUpMacCompanion={setUpMacCompanion} /></WorkspacePane> : null}
        {visitedDestinations.has('Inbox') ? <WorkspacePane active={destination === 'Inbox'} style={styles.inboxWorkspace}>{(!usesCompactInbox || !compactChatOpen) ? <ConversationPane compact={usesCompactInbox} width={conversationPaneWidth} selectedId={selectedConversationId} onSelect={openConversation} onOpenSearch={() => openGlobalSearch()} conversations={conversations} loading={loadingChats} error={dataError} onRefresh={() => { refreshChats().catch(() => undefined); }} /> : null}{!usesCompactInbox ? <PaneResizeHandle accessibilityLabel="Resize conversation list" direction={1} initialWidth={conversationPaneWidth} onWidthChange={(next) => setConversationPaneWidth(clampDesktopPaneWidth(next, 'conversation'))} /> : null}{(!usesCompactInbox || compactChatOpen) ? <ChatPane api={api} compact={usesCompactInbox} onBack={usesCompactInbox ? () => setCompactChatOpen(false) : undefined} selected={selected} messages={messages} highlightedMessageId={highlightedMessageId} apiBaseUrl={auth.config.apiUrl} loading={loadingMessages} loadingOlder={loadingOlderMessages} hasMoreMessages={hasMoreMessages} draft={draft} onDraftChange={setDraft} composerFocusRequest={composerFocusRequest} onLoadOlder={() => { loadOlderMessages().catch(() => undefined); }} onSend={() => { sendDraft().catch(() => undefined); }} onAskClaire={() => { if (canShowInspector) { setInspectorMode('assistant'); setInspectorCollapsed(false); } else openGlobalSearch(); }} onOpenPerson={openPerson} assistantPanelAvailable={canShowInspector} assistantPanelVisible={showsInspector} onToggleAssistantPanel={() => setInspectorCollapsed((current) => !current)} error={dataError} /> : null}{showsInspector ? <><PaneResizeHandle accessibilityLabel="Resize conversation inspector" direction={-1} initialWidth={inspectorPaneWidth} onWidthChange={(next) => setInspectorPaneWidth(clampDesktopPaneWidth(next, 'inspector'))} />{inspectorMode === 'assistant' ? <ConversationAssistantInspector api={api} selected={selected} width={inspectorPaneWidth} onCollapse={() => setInspectorCollapsed(true)} onOpenMessage={(chatId, messageId) => { setSelectedConversationId(chatId); setHighlightedMessageId(messageId); }} /> : <ConversationContactInspector api={api} selected={selected} width={inspectorPaneWidth} onOpenAssistant={() => setInspectorMode('assistant')} onOpenPerson={openPerson} />}</> : null}</WorkspacePane> : null}
        </View>
      </View>
    </SafeAreaView>
  );
...
function WorkspacePane({ active, children, style }: { active: boolean; children: React.ReactNode; style?: React.ComponentProps<typeof View>['style'] }) {
  return <View accessibilityElementsHidden={!active} pointerEvents={active ? 'auto' : 'none'} style={active ? [styles.workspacePane, style] : styles.workspacePaneHidden}>{children}</View>;
}

function DesktopWrapGrid({ children, minItemWidth = 320, gap = space[3] }: { children: React.ReactNode; minItemWidth?: number; gap?: number }) {
  const { width: windowWidth } = useWindowDimensions();
  const [width, setWidth] = useState(0);
  const items = React.Children.toArray(children).filter(Boolean);
  const { columns, itemWidth } = desktopWrapGridMetrics(width || Math.max(minItemWidth, windowWidth - 420), items.length, minItemWidth, gap);
  return <View onLayout={(event) => setWidth(event.nativeEvent.layout.width)} style={desktopWrapGridStyles.row}>{items.map((child, index) => <View key={index} style={[desktopWrapGridStyles.item, itemWidth ? { width: itemWidth, marginRight: (index + 1) % columns === 0 ? 0 : gap, marginBottom: gap } : null]}>{child}</View>)}</View>;
}
...
function NavigationRail({ compact, destination, onSelect }: { compact: boolean; destination: Destination; onSelect: (value: Destination) => void }) {
  ...
  const entries: Destination[] = ['Home', 'Inbox', 'Loops', 'Search', 'People', 'Settings'];
...
function NavigationIcon({ entry, active }: { entry: Destination; active: boolean }) {
  ...
  const Icon = entry === 'Home' ? Home : entry === 'Inbox' ? InboxIcon : entry === 'Loops' ? ListTodo : entry === 'People' ? Users : entry === 'Search' ? Sparkles : Settings;
...
function loopConversation(loop: DesktopLoop) {
  return loop.chat?.name || loop.contact?.name || loop.contact?.inferred_name || loop.contact_name || 'Conversation';
}
...
function LoopsPane({ api, onOpenChat, onOpenInbox }: { api: ClaireApi | null; onOpenChat: (chatId: string) => void; onOpenInbox: () => void }) {
  const [loops, setLoops] = useState<DesktopLoop[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  ...
  const refresh = useCallback(async () => {
    if (!api) return;
    setLoading(true);
    try { setLoops(await api.getLoops()); setError(null); } catch (loadError) { setError(loadError instanceof Error ? loadError.message : 'Unable to load loops.'); } finally { setLoading(false); }
  }, [api]);
  useEffect(() => { refresh().catch(() => undefined); }, [refresh]);
  const open = loops.filter((item) => ['open', 'waiting', 'snoozed'].includes(item.status));
  const overdueCount = open.filter((item) => Boolean(item.deadline && new Date(item.deadline).getTime() < Date.now())).length;
  const snoozeUntil = () => {
  ...
  const update = async (loop: DesktopLoop, action: 'complete' | 'snooze') => {
    if (!api || updatingId) return;
    setUpdatingId(loop.id);
    setError(null);
    try {
      const updated = action === 'complete'
        ? await api.updateLoop(loop.id, { status: 'done' })
        : await api.snoozeLoop(loop.id, snoozeUntil());
      setLoops((current) => current.map((item) => item.id === loop.id ? { ...item, ...updated } : item));
    } catch (updateError) {
      setError(updateError instanceof Error ? updateError.message : 'Unable to update this loop.');
    } finally {
      setUpdatingId(null);
    }
  };
  return <ScrollView style={styles.loopsPane} contentContainerStyle={desktopLoopsStyles.content}>
    <View style={desktopLoopsStyles.header}>
      <View style={desktopLoopsStyles.headerCopy}><ClaireText variant="monoLabel" style={styles.dailyBriefDate}>FOLLOW-THROUGH</ClaireText><ClaireText variant="display">Loops</ClaireText><ClaireText variant="body" style={styles.muted}>{open.length ? `${open.length} commitments are waiting for your next move.` : 'Claire will surface commitments from your conversations here.'}</ClaireText></View>
      <ClaireIconButton accessibilityLabel="Refresh loops" disabled={loading} onPress={() => { refresh().catch(() => undefined); }}>{loading ? <ActivityIndicator size="small" color={colors.ink} /> : <RefreshCw size={18} color={colors.ink} />}</ClaireIconButton>
    </View>
    {!loading && !error ? <View style={desktopLoopsStyles.summary}><ClaireStatusPill tone={overdueCount ? 'warning' : 'success'}>{overdueCount ? `${overdueCount} overdue` : 'On track'}</ClaireStatusPill><ClaireText variant="bodySmall" style={styles.muted}>{open.length} open · {loops.filter((item) => item.status === 'done').length} completed</ClaireText></View> : null}
    {loading ? <LoadingRow label="Loading loops…" /> : null}
    {error ? <View style={desktopLoopsStyles.error}><ClaireText variant="bodySmall" style={styles.errorText}>{error}</ClaireText><ClaireButton variant="quiet" onPress={() => { refresh().catch(() => undefined); }}>Try again</ClaireButton></View> : null}
    {!loading && !error && !open.length ? <ClaireCard tone="paper" style={desktopLoopsStyles.empty}><View style={desktopLoopsStyles.emptyMark}><ListTodo size={24} color={colors.ink} /></View><ClaireText variant="sectionTitle">Nothing open right now</ClaireText><ClaireText variant="body" style={styles.muted}>When a conversation contains a commitment, Claire will keep it here with a direct path back to the chat.</ClaireText><ClaireButton onPress={onOpenInbox}>Open Inbox</ClaireButton></ClaireCard> : null}
    <View style={desktopLoopsStyles.list}>{open.map((loop) => {
      const conversation = loopConversation(loop);
      const avatarUrl = loop.contact?.avatar_url || loop.chat?.contact?.avatar_url || undefined;
      const overdue = Boolean(loop.deadline && new Date(loop.deadline).getTime() < Date.now());
      const isUpdating = updatingId === loop.id;
      return <Pressable key={loop.id} accessibilityRole={loop.chat_id ? 'button' : undefined} accessibilityLabel={loop.chat_id ? `Open ${conversation} conversation for loop: ${loop.content}` : undefined} disabled={!loop.chat_id} onPress={() => loop.chat_id && onOpenChat(loop.chat_id)} style={({ pressed }) => [desktopLoopsStyles.card, overdue && desktopLoopsStyles.cardOverdue, pressed && loop.chat_id && styles.pressed]}>
        <View style={desktopLoopsStyles.cardHead}><View style={desktopLoopsStyles.person}><ClaireAvatar initials={initials(conversation)} source={avatarUrl ? { uri: avatarUrl } : undefined} tone="sky" size={42} /><View style={desktopLoopsStyles.personCopy}><ClaireText variant="body" style={styles.conversationName}>{conversation}</ClaireText><ClaireText variant="monoLabel" style={styles.platformLabel}>{loop.platform || loop.chat?.platform || 'Conversation'}</ClaireText></View></View><View onStartShouldSetResponder={() => true} style={desktopLoopsStyles.cardActions}>
          <ClaireIconButton accessibilityLabel={`Mark “${loop.content}” complete`} disabled={Boolean(updatingId)} onPress={() => { update(loop, 'complete').catch(() => undefined); }}>{isUpdating ? <ActivityIndicator size="small" color={colors.ink} /> : <ListTodo size={17} color={colors.ink} />}</ClaireIconButton>
          <ClaireIconButton accessibilityLabel={`Snooze “${loop.content}” until tomorrow`} disabled={Boolean(updatingId)} onPress={() => { update(loop, 'snooze').catch(() => undefined); }}><RefreshCw size={16} color={colors.neutral[600]} /></ClaireIconButton>
        </View></View>
        <ClaireText variant="sectionTitle" style={desktopLoopsStyles.loopText}>{loop.content}</ClaireText>
        <View style={desktopLoopsStyles.cardFoot}><ClaireStatusPill tone={overdue ? 'warning' : 'info'}>{deadlineLabel(loop.deadline) || 'No due date'}</ClaireStatusPill><ClaireText variant="label" style={styles.replyLabel}>{loop.chat_id ? 'Open conversation' : 'Conversation unavailable'}</ClaireText></View>
      </Pressable>;
    })}</View>
  </ScrollView>;
}
...
function HomePane({ api, companion, conversations, onOpenChat, onOpenInbox }: { api: ClaireApi | null; companion: CompanionStatus | null; conversations: Conversation[]; onOpenChat: (chatId: string) => void; onOpenInbox: () => void }) {
  const [loops, setLoops] = useState<DesktopLoop[]>([]);
  const [loadingLoops, setLoadingLoops] = useState(Boolean(api));
  ...
  useEffect(() => {
    ...
    api.getLoops().then((next) => active && setLoops(next)).catch(() => undefined).finally(() => active && setLoadingLoops(false));
    return () => { active = false; };
  }, [api]);
  useEffect(() => {
    let active = true;
  ...
  const openLoops = loops.filter((item) => ['open', 'waiting', 'snoozed'].includes(item.status)).slice(0, 3);
  ...
  return <ScrollView style={styles.surfacePane} contentContainerStyle={[styles.dailyBriefContent, desktopHomeStyles.content]}>
    ...
    <View style={[desktopHomeStyles.row, stacked && desktopHomeStyles.rowStacked]}>
      ...
      <ClaireCard tone="paper" style={[desktopHomeStyles.card, desktopHomeStyles.loops, stacked && desktopHomeStyles.cardStacked]}><ClaireText variant="monoLabel" style={styles.dailyBriefDate}>OPEN LOOPS</ClaireText>{loadingLoops ? <LoadingRow label="Loading loops…" /> : openLoops.length ? openLoops.map((loop) => <Pressable key={loop.id} accessibilityRole={loop.chat_id ? 'button' : undefined} disabled={!loop.chat_id} onPress={() => loop.chat_id && onOpenChat(loop.chat_id)} style={({ pressed }) => [desktopHomeStyles.actionRow, pressed && loop.chat_id && styles.pressed]}><View style={styles.loopDot} /><View style={styles.dailyBriefRowCopy}><ClaireText variant="body" style={desktopHomeStyles.actionTitle}>{loop.content}</ClaireText><ClaireText variant="bodySmall" style={desktopHomeStyles.actionDetail}>{deadlineLabel(loop.deadline) || 'Open commitment'}</ClaireText></View><ClaireText variant="label" style={styles.replyLabel}>Open</ClaireText></Pressable>) : <ClaireText variant="body" style={styles.muted}>No open loops right now.</ClaireText>}</ClaireCard>
    </View>
  </ScrollView>;
}
...
function AssistantPane({ api, selected, initialQuestion, onInitialQuestionConsumed, onOpenMessage }: { api: ClaireApi | null; selected: Conversation | null; initialQuestion: string | null; onInitialQuestionConsumed: () => void; onOpenMessage: (chatId: string, messageId: string) => void }) {
  ...
  const tools = [
    ...
    { title: 'Find open loops', detail: 'Commitments and unanswered questions.', prompt: 'What open loops or commitments need my attention?', icon: ListTodo },
...
function ConnectionsPane({ companion, companionNotice, api, apiUrl, accessToken, onSetUpMacCompanion, embedded = false }: { companion: CompanionStatus | null; companionNotice: string | null; api: ClaireApi | null; apiUrl: string; accessToken: string; onSetUpMacCompanion: () => Promise<void>; embedded?: boolean }) {
  ...
  return <ScrollView style={embedded ? desktopConnectionsStyles.embeddedScroll : styles.surfacePane} contentContainerStyle={[styles.connectionsContent, desktopConnectionsStyles.content, embedded && desktopConnectionsStyles.embeddedContent]}>
    ...
    {definitions.length ? <><DesktopWrapGrid minItemWidth={300}>{activeDefinitions.map((item) => <ConnectionOverviewCard key={item.id} definition={item} selected={item.id === selectedPlatformId} summary={summaryFor(item)} requested={false} onPress={() => { setSelectedPlatformId(item.id); setInterestNotice(null); }} />)}</DesktopWrapGrid>
      {roadmapDefinitions.length ? <View style={desktopConnectionsStyles.roadmap}><View><ClaireText variant="monoLabel" numberOfLines={1} style={styles.contextLabel}>ROADMAP</ClaireText><ClaireText variant="sectionTitle">Request a platform</ClaireText><ClaireText variant="bodySmall" style={styles.muted}>Requests express interest only—Claire never asks for credentials here.</ClaireText></View><DesktopWrapGrid minItemWidth={300}>{roadmapDefinitions.map((item) => <ConnectionOverviewCard key={item.id} definition={item} selected={item.id === selectedPlatformId} requested={requestedPlatformIds.includes(item.id)} onPress={() => { setSelectedPlatformId(item.id); setInterestNotice(null); }} />)}</DesktopWrapGrid></View> : null}
...
function SettingsPane({ api, companion, companionNotice, apiUrl, accessToken, userId, cache, fullHistoryEnabled, onFullHistoryChange, onNotificationPreferenceChange, onSetUpMacCompanion }: { api: ClaireApi | null; companion: CompanionStatus | null; companionNotice: string | null; apiUrl: string; accessToken: string; userId: string; cache: DesktopCache; fullHistoryEnabled: boolean; onFullHistoryChange: (enabled: boolean) => void; onNotificationPreferenceChange: (enabled: boolean) => void; onSetUpMacCompanion: () => Promise<void> }) {
  ...
  const shortcutDefaults: Record<string, string> = { Home: '⌘1', Inbox: '⌘2', Loops: '⌘3', People: '⌘4', 'Ask Claire': '⌘K', Search: '⌘⇧F', Settings: '⌘,', 'New message': '⌘N' };
...
function ConversationContactInspector({ api, selected, width, onOpenAssistant, onOpenPerson }: { api: ClaireApi | null; selected: Conversation | null; width: number; onOpenAssistant: () => void; onOpenPerson: (chatId: string) => void }) {
  ...
  useEffect(() => {
    let active = true;
    setSettings(null); setLoops([]);
    if (!api || !selectedId) return () => { active = false; };
    Promise.all([api.getConversationSettings(selectedId), api.getLoops()])
      .then(([nextSettings, nextLoops]) => {
        if (!active) return;
        setSettings(nextSettings);
        setLoops(nextLoops.filter((item) => item.chat_id === selectedId && item.status !== 'done').slice(0, 3));
      })
  ...
  return <View style={[styles.inspector, desktopContactInspectorStyles.root, { width }]} testID="conversation-contact-inspector">
    ...
    <ScrollView contentContainerStyle={desktopContactInspectorStyles.content}>
      ...
      <View style={desktopContactInspectorStyles.section}><ClaireText variant="monoLabel" style={styles.dailyBriefDate}>OPEN LOOPS</ClaireText>{loops.length ? loops.map((loop) => <Pressable key={loop.id} accessibilityRole="button" onPress={() => onOpenPerson(selected.id)} style={({ pressed }) => [desktopContactInspectorStyles.loop, pressed && styles.pressed]}><View style={desktopContactInspectorStyles.loopDot} /><View style={desktopContactInspectorStyles.loopCopy}><ClaireText variant="bodySmall" numberOfLines={2}>{loop.content}</ClaireText><ClaireText variant="bodySmall" style={styles.muted}>{loop.deadline ? new Date(loop.deadline).toLocaleDateString() : 'No due date'}</ClaireText></View></Pressable>) : <ClaireText variant="bodySmall" style={styles.muted}>No open loops in this conversation.</ClaireText>}</View>
...
const desktopShellStyles = StyleSheet.create({
  ...
  titleBarSearchInput: { flex: 1, minWidth: 0, height: 30, color: colors.ink, fontFamily: fonts.sans, fontSize: 13, lineHeight: 18, paddingTop: 6, paddingBottom: 6 },
  ...
  navigationRail: { paddingTop: 40, flexGrow: 0, flexShrink: 0 },
  navigationRailCompact: { width: 84, paddingTop: 32, flexGrow: 0, flexShrink: 0 },
...
const desktopAskStyles = StyleSheet.create({
  ...
  threadRail: { width: 270, flexGrow: 0, flexShrink: 0, backgroundColor: '#fcfcfb', borderRightWidth: 1, borderRightColor: colors.neutral[200], padding: space[3] },
  ...
  composerInput: { flex: 1, minWidth: 0, color: colors.ink, fontFamily: fonts.sans, fontSize: 16, lineHeight: 22, paddingTop: 18, paddingBottom: 18 },
  composerSend: { width: 40, height: 40, borderRadius: 14, backgroundColor: colors.ink },
  evidence: { width: 280, flexGrow: 0, flexShrink: 0, backgroundColor: '#fcfcfb', borderLeftWidth: 1, borderColor: colors.neutral[200], padding: space[4], paddingTop: space[6], rowGap: space[3] },
  evidenceCard: { padding: space[3], rowGap: space[2] },
});

const desktopSettingsStyles = StyleSheet.create({
  workspace: { flex: 1, flexDirection: 'row', minWidth: 0, backgroundColor: colors.cream },
  sidebar: { width: 280, flexGrow: 0, flexShrink: 0, backgroundColor: '#fcfcfb', borderRightWidth: 1, borderColor: colors.neutral[200], paddingHorizontal: space[5], paddingVertical: space[6], rowGap: space[1] },
  ...
  shortcutInput: { width: 76, color: colors.ink, fontFamily: fonts.mono, fontSize: 13, lineHeight: 18, textAlign: 'right', paddingTop: 8, paddingBottom: 8 },
  errorBanner: { borderRadius: radius.control, backgroundColor: colors.warningSurface, paddingHorizontal: space[3], paddingVertical: space[2] },
});
...
const desktopContactInspectorStyles = StyleSheet.create({
  ...
  loop: { minHeight: 52, flexDirection: 'row', alignItems: 'flex-start', columnGap: space[2], borderBottomWidth: 1, borderColor: colors.neutral[200], paddingVertical: space[2] },
  loopDot: { width: 14, height: 14, marginTop: 3, borderRadius: 7, borderWidth: 2, borderColor: colors.warning },
  loopCopy: { flex: 1, minWidth: 0, rowGap: 2 },
});

const desktopPeopleStyles = StyleSheet.create({
  workspace: { flex: 1, flexDirection: 'row', minWidth: 0, backgroundColor: colors.cream },
  workspaceNarrow: { flexDirection: 'column' },
  listPane: { width: 300, flexGrow: 0, flexShrink: 0, backgroundColor: '#fcfcfb', borderRightWidth: 1, borderRightColor: colors.neutral[200], paddingHorizontal: space[4], paddingTop: space[5], paddingBottom: space[3] },
  ...
  searchInput: { flex: 1, minWidth: 0, color: colors.ink, fontFamily: fonts.sans, fontSize: 16, lineHeight: 21, paddingTop: 14, paddingBottom: 14 },
  ...
  editor: { flex: 1, minWidth: 0, backgroundColor: colors.cream },
  ...
  memoryInput: { minHeight: 112, borderWidth: 1, borderColor: colors.neutral[200], borderRadius: radius.card, backgroundColor: colors.paper, color: colors.ink, fontFamily: fonts.sans, fontSize: 16, lineHeight: 23, textAlignVertical: 'top', padding: space[3] },
  ...
  instructionInput: { minHeight: 88, borderWidth: 1, borderColor: colors.neutral[200], borderRadius: radius.control, backgroundColor: colors.paper, color: colors.ink, fontFamily: fonts.sans, fontSize: 15, lineHeight: 22, textAlignVertical: 'top', padding: space[3] },
  previewPane: { width: 320, flexGrow: 0, flexShrink: 0, backgroundColor: '#acd2fb', paddingHorizontal: space[4], paddingTop: space[8], paddingBottom: space[4], rowGap: space[4] },
...
const desktopConnectionsStyles = StyleSheet.create({
  ...
  card: { minHeight: 212, alignSelf: 'stretch', backgroundColor: colors.paper, borderWidth: 1, borderColor: colors.neutral[200], borderRadius: radius.card, padding: space[4], justifyContent: 'space-between', rowGap: space[3] },
  cardLocal: { backgroundColor: '#fff8dd', borderColor: '#d5bd68' },
  cardSelected: { borderColor: colors.ink },
  cardStatusSuccess: { color: colors.success },
  ...
  cardStatus: { color: colors.neutral[600], textAlign: 'right', maxWidth: 110 },
...
const desktopWrapGridStyles = StyleSheet.create({
...
const desktopHomeStyles = StyleSheet.create({
  ...
  hero: { flex: 1, minHeight: 276, justifyContent: 'space-between', backgroundColor: colors.sky },
  ...
  supportCard: { flex: 1, minHeight: 228 },
  loops: { minHeight: 228 },
...
const desktopLoopsStyles = StyleSheet.create({
  ...
  loopText: { fontSize: 19, lineHeight: 26 },
