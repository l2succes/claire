...
async function main() {
  ...
  const items = JSON.parse(await run(['op', 'item', 'list', '--vault', vault, '--format', 'json'])) as ItemIndex[];
  const sourceItems = items.filter((item) => item.title === sourceTitle);
  if (sourceItems.length !== 1) {
    throw new Error(`Expected exactly one ${sourceTitle} item in ${vault}; found ${sourceItems.length}. Resolve duplicates before provisioning the API.`);
  }
  const existingTargets = items.filter((item) => item.title === targetTitle);
  if (existingTargets.length > 0 && !rotate) {
    throw new Error(`${targetTitle} already exists. Use --rotate only for a deliberate staging API credential rotation.`);
  }
  if (rotate) {
    for (const item of existingTargets) await run(['op', 'item', 'delete', item.id, '--vault', vault], undefined, true);
  }

  const source = JSON.parse(await run(['op', 'item', 'get', sourceItems[0].id, '--format', 'json'])) as Item;
  const variables = {
  ...
  const item = {
    ...
    fields: [
      ...
      ...Object.entries(variables).map(([label, value]) => ({ label, type: 'CONCEALED', value })),
    ],
