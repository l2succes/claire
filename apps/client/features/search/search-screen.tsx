    ...
    return (
      <View testID="search-results-screen" style={{ flex: 1, backgroundColor: colors.cream }}>
        ...
        <FlatList
          ...
          ListHeaderComponent={<View style={{ gap: space[4], paddingBottom: space[3] }}>
            {semantic.isLoading ? <View style={{ minHeight: 100, borderRadius: radius.card, backgroundColor: colors.lavender, alignItems: 'center', justifyContent: 'center' }}><ActivityIndicator color={colors.ink} /></View> : semantic.data ? (
              <View style={{ padding: space[4], gap: space[2], borderRadius: radius.card, borderCurve: 'continuous', borderWidth: 1, borderColor: colors.ink, backgroundColor: colors.lavender }}>
                ...
                <View style={{ gap: space[2] }}>{semantic.data.citations.slice(0, 3).map(citation => <FeedbackPressable key={citation.messageId} onPress={() => router.push({ pathname: '/chat/[chatId]', params: { chatId: citation.chatId, chat_name: citation.chatName || '', platform: citation.platform, is_group: citation.isGroup ? '1' : '0', highlightMessageId: citation.messageId } })} style={({ pressed }) => ({ padding: space[3], borderRadius: radius.control, backgroundColor: colors.paper, opacity: pressed ? 0.68 : 1 })}><Text style={{ ...mobileType.label, color: colors.ink }}>{citation.chatName || citation.senderName} · {citation.platform}</Text><Text numberOfLines={2} style={{ ...mobileType.bodySmall, color: colors.neutral[600] }}>{citation.excerpt}</Text></FeedbackPressable>)}</View>
          ...
          renderItem={({ item }) => <FeedbackPressable testID={`search-result-${item.kind}-${item.id}`} onPress={item.onPress} style={({ pressed }) => ({ minHeight: 70, flexDirection: 'row', alignItems: 'center', gap: space[3], paddingVertical: space[3], borderBottomWidth: 1, borderBottomColor: colors.neutral[200], opacity: pressed ? 0.65 : 1 })}>
