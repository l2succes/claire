...
export function InstagramLoginPreviewScreen() {
  ...
  const live = fixture !== '1' && process.env.EXPO_PUBLIC_INSTAGRAM_LOGIN_LIVE_PREVIEW === 'true';
  if (!__DEV__ || (!live && fixture !== '1' && process.env.EXPO_PUBLIC_INSTAGRAM_LOGIN_PREVIEW !== 'true')) return null;
  return <View style={{ flex: 1, padding: 30, paddingTop: 100, gap: 20 }}>
    ...
    <Button title={live ? 'Open real Instagram sign-in' : 'Open synthetic sign-in'} disabled={!hasInstagramMobileLogin} onPress={async () => {
      try { setOutcome(JSON.stringify(await startInstagramMobileLogin(fixture === '1' ? 'http://127.0.0.1:3310' : 'http://127.0.0.1:3309', live ? 'local-live-instagram' : 'synthetic-fixture'))); }
