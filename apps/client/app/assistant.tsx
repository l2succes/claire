...
function DesktopAssistantWorkspace({ threads, activeThread, turns, loading, question, asking, error, onQuestionChange, onCreate, onSelect, onAsk }: {
  ...
  return <View style={{ flex: 1, minHeight: 0, flexDirection: 'row', backgroundColor: '#FAF9F5' }} testID="desktop-assistant-screen">
    ...
    <View style={{ width: 235, flexShrink: 0, padding: space[3], borderLeftWidth: 1, borderColor: colors.neutral[200], backgroundColor: '#F4F2EC' }}><Text style={{ ...mobileType.monoLabel, color: colors.neutral[600] }}>CLAIRE’S CONTEXT</Text><DesktopEvidence label="RELATIONSHIP" title="Context-aware" detail="Claire uses the conversation and the relationship memory you’ve chosen." /><DesktopEvidence label="OPEN PROMISES" title="Find open loops" detail="Promises and unresolved plans are available to ask about." /><View style={{ marginTop: 12, padding: 12, borderRadius: 12, borderWidth: 1, borderColor: '#DFCA7E', backgroundColor: '#FFF8DD' }}><Text style={{ ...mobileType.monoLabel, color: colors.ink }}>AVAILABLE SOURCES</Text><Text style={{ ...mobileType.bodySmall, fontWeight: '700', color: colors.ink, marginTop: 5 }}>Connected messages</Text><Text style={{ ...mobileType.label, color: colors.neutral[600], marginTop: 3 }}>Claire shows the messages behind any answer.</Text></View></View>
  </View>;
}
