  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          data: { name },
          // Uses the same native/web/Electron-aware callback as Google OAuth.
          emailRedirectTo: getAuthRedirectUri(),
        },
      });

      if (error) {
        Alert.alert('Signup Error', userFacingErrorMessage(error, 'Failed to sign up'));
        return;
      }
      ...
      Alert.alert('Error', userFacingErrorMessage(error, 'Failed to sign up'));
    } finally {
      setLoading(false);
    }
  };
