...
type BridgeSessionState = 'connected' | 'setup' | 'attention' | 'superseded' | 'retired';
type BridgeSession = { accountRef: string; platform: string; state: BridgeSessionState; reason: string; recovery: string; lastConnectedAt: string | null; statusChangedAt: string | null; isCurrent: boolean; canRetire: boolean };
...
const bridgeStateStatus: Record<BridgeSessionState, Status> = { connected: 'healthy', setup: 'unknown', attention: 'warning', superseded: 'unknown', retired: 'unknown' };
const bridgeStateCopy: Record<BridgeSessionState, string> = { connected: 'connected', setup: 'setup in progress', attention: 'needs a decision', superseded: 'superseded', retired: 'retired' };
...
function BridgeModule({ data, attentionOnly, onClearAttention }: { data: BridgeData | null; attentionOnly: boolean; onClearAttention: () => void }) {
  ...
  const visibleSessions = (data?.sessions || []).filter((session) => session.state !== 'retired' && (!attentionOnly || session.state === 'attention'));
  ...
  return <>
    ...
    <section className="mt-7 border border-[#c8c8c0] bg-paper p-5 sm:p-8"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-2xl font-semibold tracking-[-.035em]">Bridge connection health</h2><p className="mt-2 text-neutral-600">Current connection state, exact timestamps, and privacy-safe recovery guidance.</p></div><span className="font-mono text-xs uppercase text-neutral-600">{visibleSessions.length} visible connections</span></div><div className="mt-6 overflow-x-auto"><table className="w-full min-w-[920px] border-collapse text-left"><thead className="border-y border-neutral-200 font-mono text-xs uppercase text-neutral-600"><tr><th className="px-2 py-3 font-medium">Account ref</th><th className="px-2 py-3 font-medium">Platform</th><th className="px-2 py-3 font-medium">State</th><th className="px-2 py-3 font-medium">Last connected</th><th className="px-2 py-3 font-medium">Status updated</th><th className="px-2 py-3 font-medium">Why / what to do</th></tr></thead><tbody>{visibleSessions.map((session) => <tr className="border-b border-neutral-200 align-top" key={`${session.platform}:${session.accountRef}`}><td className="px-2 py-4 font-mono text-sm">acct_{session.accountRef.slice(0, 8)}</td><td className="px-2 py-4 capitalize">{session.platform}</td><td className="px-2 py-4"><span className="inline-flex items-center gap-2"><StatusDot status={bridgeStateStatus[session.state]} /><span className="font-mono text-xs uppercase">{bridgeStateCopy[session.state]}</span></span></td><td className="px-2 py-4 font-mono text-sm" title={absoluteTime(session.lastConnectedAt)}>{relativeTime(session.lastConnectedAt)}</td><td className="px-2 py-4 font-mono text-sm" title={absoluteTime(session.statusChangedAt)}>{relativeTime(session.statusChangedAt)}</td><td className="max-w-md px-2 py-4 text-sm"><p>{session.reason}</p><p className="mt-1 text-neutral-600">{session.recovery}</p></td></tr>)}{!visibleSessions.length && <tr><td colSpan={6} className="px-2 py-6 text-neutral-600">{attentionOnly ? 'No current bridge connections need a decision.' : 'No durable bridge connections have been recorded yet.'}</td></tr>}</tbody></table></div></section>
  </>;
}

function BridgeRecoveryPanel({
  sessions,
  traffic,
  canRetire,
  onViewDetails,
  ...
  canRetire: boolean;
  onViewDetails: () => void;
  onRetire: (accountRef: string) => Promise<void>;
}) {
  const [confirmingRef, setConfirmingRef] = useState<string | null>(null);
  const [retiringRef, setRetiringRef] = useState<string | null>(null);
  const livePlatforms = traffic
    .filter((platform) => platform.inbound + platform.outbound > 0 && platform.failed === 0)
    .map((platform) => platform.platform);

  if (!sessions.length) return null;

  const retire = async (accountRef: string) => {
    setRetiringRef(accountRef);
    try {
      await onRetire(accountRef);
      setConfirmingRef(null);
    } catch {
      // The parent renders the API error above the dashboard.
    } finally {
      setRetiringRef(null);
    }
  };

  return <section className="mt-9 border-2 border-[#d89631] bg-[#fff3df] p-5 sm:p-8" aria-labelledby="bridge-recovery-heading">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div>
        <p className="font-mono text-xs font-semibold uppercase tracking-[.16em] text-[#8a5300]">Connection decision required</p>
        <h2 id="bridge-recovery-heading" className="mt-1 text-3xl font-semibold tracking-[-.045em]">{sessions.length} connection{sessions.length === 1 ? '' : 's'} need{sessions.length === 1 ? 's' : ''} a decision</h2>
        <p className="mt-3 max-w-3xl text-lg text-neutral-700">These are current expected connections that are disconnected or failed—not a count of every historical login attempt.</p>
        {livePlatforms.length > 0 && <p className="mt-2 font-semibold text-[#277d41]">Live traffic is still flowing without failures on {livePlatforms.map((platform) => platform[0].toUpperCase() + platform.slice(1)).join(', ')}.</p>}
      </div>
      <button type="button" className="shrink-0 border border-ink bg-paper px-4 py-3 font-semibold hover:bg-neutral-100" onClick={onViewDetails}>Open bridge details →</button>
    </div>
    <div className="mt-6 grid gap-4 xl:grid-cols-2">
      {sessions.map((bridgeSession) => <article className="border border-[#d89631] bg-paper p-5" key={bridgeSession.accountRef}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="font-semibold capitalize">{bridgeSession.platform} · <span className="font-mono">acct_{bridgeSession.accountRef.slice(0, 8)}</span></p>
            <p className="mt-1 font-mono text-sm text-neutral-600" title={absoluteTime(bridgeSession.statusChangedAt)}>Status changed {relativeTime(bridgeSession.statusChangedAt)}</p>
          </div>
          <StatusLabel status="warning" />
        </div>
        <p className="mt-4">{bridgeSession.reason}</p>
        <p className="mt-2 text-neutral-600">{bridgeSession.recovery}</p>
        <div className="mt-5 border-t border-neutral-200 pt-4">
          {confirmingRef === bridgeSession.accountRef ? <div className="flex flex-wrap items-center gap-3">
            <p className="basis-full text-sm"><strong>Retire this connection?</strong> It and older failed attempts for the same platform will stop affecting health alerts. Messages and connection history are not deleted.</p>
            <button type="button" className="border border-danger bg-danger px-3 py-2 text-sm font-semibold text-paper disabled:opacity-50" disabled={retiringRef === bridgeSession.accountRef} onClick={() => void retire(bridgeSession.accountRef)}>{retiringRef === bridgeSession.accountRef ? 'Retiring…' : 'Confirm retirement'}</button>
            <button type="button" className="border border-ink px-3 py-2 text-sm font-semibold" disabled={retiringRef === bridgeSession.accountRef} onClick={() => setConfirmingRef(null)}>Cancel</button>
          </div> : canRetire && bridgeSession.canRetire
            ? <button type="button" className="text-sm font-semibold text-danger underline decoration-1 underline-offset-4" onClick={() => setConfirmingRef(bridgeSession.accountRef)}>Retire from health monitoring</button>
            : <p className="text-sm text-neutral-600">An Operations owner can retire this connection if it is no longer expected.</p>}
        </div>
      </article>)}
    </div>
  </section>;
...
export function OperationsConsole() {
  ...
  const retireBridge = async (accountRef: string) => {
    setError('');
    try {
      await request('/bridges/retire', { method: 'POST', body: JSON.stringify({ accountRef }) });
      await load(true);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Could not retire the platform connection');
      throw cause;
    }
  };
