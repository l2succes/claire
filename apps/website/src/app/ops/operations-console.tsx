...
type BridgePlatform = { id: string; name: string; mark: string; bridge: string; supportStatus: 'available' | 'beta' | 'planned' | 'unavailable'; setupLabel: string; runtimeLabel: string; detail: string; flow: string[]; connected: number; setup: number; attention: number; lastActivityAt: string | null };
  ...
  return <>
    <section className="mt-9 border border-[#c8c8c0] bg-paper p-5 sm:p-8">
      ...
      <div className="mt-7 grid gap-7 xl:grid-cols-[1.1fr_.9fr]">
        <div><h3 className="text-xl font-semibold">Supported network roadmap</h3><div className="mt-4 grid gap-3 sm:grid-cols-2">{data?.platforms.map((platform) => <button type="button" key={platform.id} onClick={() => setSelectedId(platform.id)} className={`border p-4 text-left transition ${selected?.id === platform.id ? 'border-2 border-ink bg-[#f8f7f2]' : 'border-neutral-200 hover:border-ink'}`}><div className="flex items-start justify-between gap-3"><span className="grid size-9 place-items-center bg-ink font-mono text-xs font-semibold text-paper">{platform.mark}</span><span className={`font-mono text-xs font-semibold uppercase ${platform.supportStatus === 'available' ? 'text-[#277d41]' : platform.supportStatus === 'beta' ? 'text-[#a86000]' : 'text-neutral-600'}`}>{platform.supportStatus === 'available' ? 'live' : platform.supportStatus}</span></div><p className="mt-4 font-semibold">{platform.name}</p><p className="mt-1 font-mono text-xs text-neutral-600">{platform.bridge}</p><p className="mt-3 text-sm text-neutral-600">{platform.connected} connected · {platform.attention} attention</p></button>)}{!data && <p className="text-neutral-600">Loading supported platforms…</p>}</div></div>
        <aside className="border border-ink bg-[#f8f7f2] p-5"><p className="font-mono text-xs font-semibold uppercase tracking-[.14em] text-neutral-600">Selected flow</p>{selected ? <><div className="mt-3 flex items-center gap-3"><span className="grid size-11 place-items-center bg-ink font-mono text-xs font-semibold text-paper">{selected.mark}</span><div><h3 className="text-2xl font-semibold">{selected.name}</h3><p className="font-mono text-xs text-neutral-600">{selected.supportStatus === 'available' ? 'Live connector' : `${selected.supportStatus} connector`}</p></div></div><p className="mt-5 text-neutral-600">{selected.detail}</p><div className="mt-6 flex flex-wrap items-center gap-2">{selected.flow.map((stage, index) => <span className="flex items-center gap-2 font-mono text-xs" key={stage}><span className="border border-ink bg-paper px-2 py-2">{stage}</span>{index < selected.flow.length - 1 && <span className="text-neutral-500">→</span>}</span>)}</div><dl className="mt-6 divide-y divide-neutral-200 border-y border-neutral-200"><div className="flex justify-between gap-4 py-3"><dt>Connection setup</dt><dd className="font-mono text-sm">{selected.setupLabel}</dd></div><div className="flex justify-between gap-4 py-3"><dt>Runtime</dt><dd className="font-mono text-sm">{selected.runtimeLabel}</dd></div><div className="flex justify-between gap-4 py-3"><dt>Latest activity</dt><dd className="font-mono text-sm">{relativeTime(selected.lastActivityAt)}</dd></div></dl></> : <p className="mt-4 text-neutral-600">Choose a platform to inspect its path.</p>}</aside>
      </div>
    </section>
