...
const schema = z.object({ query: z.object({
  ...
  scope: z.enum(['everything', 'messages', 'people', 'files', 'loops']).optional().default('everything'),
...
router.get('/', requireAuth, validateRequest(schema), async (req: Request, res: Response) => {
  try {
    ...
    const [messageResponse, peopleResponse, promiseResponse, fileResponse] = await Promise.all([
      ...
      wants('loops') ? supabase.from('loops').select('id, content, deadline, status, chat_id, chat:chats!loops_chat_id_fkey(name, is_group, platform)').eq('user_id', userId).ilike('content', pattern).order('created_at', { ascending: false }).limit(limit) : Promise.resolve({ data: [], error: null }),
    ...
    return res.json({ success: true, data: {
      ...
      loops: promiseResponse.data || [],
      files: fileResponse.data || [],
      counts: { messages: messageResponse.data?.length || 0, people: peopleResponse.data?.length || 0, loops: promiseResponse.data?.length || 0, files: fileResponse.data?.length || 0 },
    } });
  } catch (error) {
