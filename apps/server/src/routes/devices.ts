...
/** Deployment diagnostic for desktop setup. Does not reveal account or fleet size. */
router.get('/readiness', async (_req: Request, res: Response) => {
  const { error } = await supabase
    .from('companion_devices')
    .select('id', { count: 'exact', head: true });
  return res.status(error ? 503 : 200).json({ ready: !error });
});
...
router.get('/', requireAuth, async (req: Request, res: Response) => {
  ...
  const { data, error } = await supabase
    .from('companion_devices')
    .select(
      'id, display_name, host_platform, capabilities, status, last_seen_at, credential_rotated_at, created_at, updated_at'
    )
    .eq('user_id', userId)
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (
      ...
      !matchesDeviceCredential(credential, device.credential_hash)
    ) {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (device.host_platform !== 'macos')
      return res.status(403).json({ error: 'Only a Mac companion can upload iMessage media' });
    if (!Buffer.isBuffer(req.body) || !req.body.length)
      return res.status(400).json({ error: 'Attachment body is required' });

    const mimeType = String(
      req.header('x-claire-media-mime-type') || 'application/octet-stream'
    ).toLowerCase();
...
router.post(
  '/:id/events',
  validateRequest(companionEventsSchema),
  async (req: Request, res: Response) => {
    const credential = req.header('x-claire-device-token');
    if (!credential) return res.status(401).json({ error: 'Missing device credential' });
